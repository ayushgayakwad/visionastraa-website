...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_admin'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($stmt->fetch()) {
            ...
            $stmt = $pdo->prepare('INSERT INTO erp_users (name, email, password, dob, phone, college_name, role, approved, batch, location) VALUES (?, ?, ?, ?, ?, ?, "admin", 1, ?, ?)');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_admin_id'])) {
    ...
    $update_sql = 'UPDATE erp_users SET name=?, email=?, dob=?, phone=?, college_name=? WHERE id=? AND role="admin"';
...
<head>
    ...
    <title>Manage Admins - Super Admin | EV Academy ERP</title>
...
<body>
        ...
        <div class="container">
            <div class="header-content" style="display:flex;align-items:center;justify-content:space-between;">
                ...
                <nav class="nav-desktop">
                    ...
                    <a href="manage_admins.php" class="nav-link active">Admins</a>
    ...
    <main>
        <div class="container">
            ...
            <section class="form-section card">
                <h2 style="color:#3a4a6b; margin-bottom: 1.5rem;">Add New Admin</h2>
            ...
            <section class="card">
                <div class="tab-btns">
                    <a href="?tab=all" class="tab-btn <?php echo ($tab === 'all' ? 'active' : ''); ?>">All Admins</a>
                    <a href="?tab=approve" class="tab-btn <?php echo ($tab === 'approve' ? 'active' : ''); ?>">Approve Admins</a>
                </div>
                <div class="responsive-actions" style="margin-bottom: 1.5rem;">
                <h2 style="color:#3a4a6b;"><?php echo ($tab === 'approve' ? 'Pending Approvals' : 'Admin List'); ?></h2>
                    <form method="GET">
                        <input type="hidden" name="tab" value="<?php echo htmlspecialchars($tab); ?>">
                        <input type="text" name="search" placeholder="Search admins..." class="form-input" style="width: 250px;" value="<?php echo htmlspecialchars($search); ?>">
    ...
    <div id="editModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000;">
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 2rem; border-radius: 14px; width: 90%; max-width: 500px;">
            <h3 style="color:#3a4a6b; margin-bottom: 1.5rem;">Edit Admin</h3>
