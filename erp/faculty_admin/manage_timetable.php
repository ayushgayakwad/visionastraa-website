...
// Get the selected week from the URL (e.g., "2024-W35"), or default to the current week.
$selected_week = $_GET['week'] ?? date('Y-\WW');

// Convert the "YYYY-Www" format to a date object representing Monday of that week.
$year = (int)substr($selected_week, 0, 4);
// **THE FIX IS HERE: Convert the week number string to an integer.**
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_slot'])) {
        ...
        $week_start_for_insert = $_POST['week_start_date']; // Get week from hidden input
        ...
        try {
            ...
            if ($e->errorInfo[1] == 1062) {
                $message = '<span style="color:red;">Error: This time slot is already booked for the selected day and week.</span>';
            } else {
                $message = '<span style="color:red;">An unexpected database error occurred: ' . $e->getMessage() . '</span>';
            }
        }
...
<head>
    ...
    <title>Manage Timetable - Faculty Admin | EV Academy ERP</title>
                ...
                <form method="POST">
                    ...
                    <div style="display:grid; gap: 1rem; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));">
                        ...
                        <div>
                            ...
                            <input type="text" name="class_name" required class="form-input" placeholder="e.g., Battery Technology">
                        </div>
                        <div>
                        ...
                        <tbody>
                            <?php foreach ($days as $day): ?>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td><?php echo $day; ?></td>
                                        <td colspan="5" style="text-align: center; color: #777;">No classes scheduled</td>
                                    </tr>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </tbody>
